...
public:
    Accelerator(
        const int &_sequense_length = 320,       // number of accelerate/gyro XYZ sent everytime
    ...
    void print_data(void){
        ...
        for (int i = 0; i < sequense_length; i++)
            if(int16_t(acce_x[i])>0){
                printf("+%d.000f, ", int16_t(acce_x[i]));
            }
            else{
                printf("%d.000f, ", int16_t(acce_x[i]));
            }
...
//         accelerator.update_current(320);
